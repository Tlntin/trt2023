...
| 8 | 2023-07-22 15:37:10 | 将union矩阵稀疏化处理，本地测试可以提高10ms左右。不过pd_score也提高了1分，需要线上测试一下得分。 | 4010.3555 | -174 | 线上效果变成了，可能和pdScore下降有关系。 | 否 |
| 9   |                     | 逐层对比CLIP模型的TensorRT fp16和onnx cpu fp32的结果，找到溢出层，实现clip fp16正常导出。经过对比，发现在fp16下，负无穷会经过softmax时会溢出。解决办法就是将-inf换成-10000即可。然后clip fp16可以导出了，并且加上了并行tokenizer，大概可以提升2ms多。 |           |              |                                                              |              |
|      |                     | plugin编写                                                   |           |              |                                                              |              |
